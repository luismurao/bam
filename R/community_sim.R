...
#' @param ... Parameters of the functions of the \code{bam} class
#' @export
#' @examples
#' \dontrun{
...
community_sim <- function(en_models,ngbs_vect,init_coords,nsteps,
  ...
  sdmsimL <- 1:n_models %>% purrr::map(function(x){
    ...
    #sparse_mod <- bam::model2sparse(model = models[[x]],
    #                                ...)
    sparse_mod <- bam::model2sparse(model = models[[x]],
                                    threshold = 0.1)
    # Compute adjacency matrix
    adj_mod <- bam::adj_mat(sparse_mod,ngbs=ngbs_vect[[x]])
    ...
    sdmsimul <- bam::sdm_sim(set_A = sparse_mod,
                             set_M = adj_mod,
